package com.bigtotoro.xlist;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

import com.example.news_android.Constants;
 ...
 * @param <T>
 */
public abstract class PageGridActivity<T> extends Activity {
	public String url = "";
	...
	public abstract View onCreatePageView(Bundle savedInstanceState);
	...
	private void createListView(){
		mPullToRefreshGridView = (PullToRefreshGridView) findViewById(R.id.pull_refresh_list);
		...
		mPullToRefreshGridView.setOnRefreshListener(new OnRefreshListener<GridView>() {
			...
			public void onRefresh(PullToRefreshBase<GridView> refreshView) {
				String label = DateUtils.formatDateTime(PageGridActivity.this, System.currentTimeMillis(),
	...
	public void load(boolean update, boolean loadMore){
		...
		Log.d(Constants.LOG, "ture_url" + ture_url);
		finalHttp.get(ture_url, new AjaxCallBack<String>(){
			...
			public void onSuccess(String t) {
				...
				if(response.getState().equals("0")){
					...
					Toast.makeText(PageGridActivity.this, response.getMsg(), Toast.LENGTH_SHORT).show();
				}
			...
			public void onFailure(Throwable t, int errorNo, String strMsg) {
				Toast.makeText(PageGridActivity.this, strMsg, Toast.LENGTH_SHORT).show();
